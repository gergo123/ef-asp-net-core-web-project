...
namespace EFTest.Rules
{
    public class RuleManager
    {
        ...
        /// <returns></returns>
        public IEnumerable<DbEntityValidationResult> ExecuteRules(System.Data.Entity.Infrastructure.DbEntityEntry entry)
        {
            if (!this.isEnabled) { return new List<DbEntityValidationResult>(); }

            var baseRuleType = System.Data.Entity.Core.Objects.ObjectContext.GetObjectType(entry.Entity.GetType());
            ...
            foreach (var ruleInstance in ruleInstances)
            {
                ...
                if (shouldValidate)
                {
                    var result = ruleInstance.ExecuteRule(entry);
                    validationResults.Add(result);
                    Debug.WriteLine($"Added {result.ValidationErrors.Count} errors");
                }
            }
